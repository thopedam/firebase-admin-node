...
function validateRetryConfig(retry: RetryConfig): void {
  ...
  if (
    ...
    !validator.isArray(retry.statusCodes)
  ) {
    throw new FirebaseAppError(
      AppErrorCodes.INVALID_ARGUMENT,
  ...
  if (
    ...
    !validator.isArray(retry.ioErrorCodes)
  ) {
    throw new FirebaseAppError(
      AppErrorCodes.INVALID_ARGUMENT,
    ...
    return AsyncHttpCall.invoke(config)
      ...
      .catch((err: LowLevelError) => {
        ...
        if (
          canRetry &&
          this.retry &&
          delayMillis <= this.retry.maxDelayInMillis
        ) {
...
export function parseHttpResponse(
  ...
  const headerLines: string[] = responseText
    .substring(0, endOfHeaderPos)
    .split("\r\n");
...
class AsyncHttpCall {
  ...
  private uncompressResponse(res: http.IncomingMessage): Readable {
    ...
    if (
      ...
      encodings.indexOf(res.headers["content-encoding"]) !== -1
    ) {
...
export class ExponentialBackoffPoller<T> extends EventEmitter {
  ...
  private getPollingDelayMillis(): number {
    const increasedPollingDelay =
      Math.pow(2, this.numTries) * this.initialPollingDelayMillis;
